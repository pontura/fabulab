...
namespace Yaguar.Auth
{
    public class FirebaseAuthManager : MonoBehaviour
    {
        ...
        IEnumerator RestoreSession()
        {
            yield return null;
            yield return null;
            if (FirebaseRestSession.Restore())
            {
                Debug.Log("#Firebase REST session restored: " + FirebaseRestSession.Uid);
                OnTokenUpdated?.Invoke();
            }
            else
                OnSignedOut?.Invoke();
        ...
        IFirebaseDBManager DB()
        {
            return firebaseDBManager.GetComponent<IFirebaseDBManager>();
        }

        public void SignOut()
        {
            FirebaseRestSession.SignOut();
            OnSignedOut?.Invoke();
        }

        public bool IsAnonymousUser()
        {
            return FirebaseRestSession.IsAnonymous;
        }

        public void SignInWithPlayGames(string authCode, System.Action<bool> callback)
        {
            Debug.LogWarning("#SignInWithPlayGames is not available in WebGL");
            callback(false);
        }

        public void LinkWithPlayGames(string username, string authCode, System.Action<bool> callback)
        {
            Debug.LogWarning("#LinkWithPlayGames is not available in WebGL");
            callback(false);
        }
        ...
        public async void SignInAnonymously()
        {
            try
            {
                await FirebaseRestSession.SignInAnonymouslyAsync();
            }
            catch (Exception e)
            {
            ...
            OnTokenUpdated?.Invoke();
            OnEmailSignedUp("", "", FirebaseRestSession.Uid);
            Debug.Log("Server: SignInAnonymously");
        }

        public async void LinkWithEmail(string username, string email, string password)
        {
            try
            {
                await FirebaseRestSession.LinkEmailAsync(email, password);
            }
            catch (Exception e)
            {
                Debug.LogError("Error al vincular email: " + e.Message);
                OnSignUp?.Invoke(false);
                return;
            }

            Debug.Log("Cuenta vinculada con email: " + email);
            OnEmailSignedUp(username, email, FirebaseRestSession.Uid);
        }
        ...
        public async void SignUpUserEmail(string username, string email, string password)
        {
            try
            {
                await FirebaseRestSession.SignUpWithEmailAsync(email, password);
            }
            catch (Exception e)
            {
        ...
        public async void LoginUserByEmail(string email, string password)
        {
            try
            {
                await FirebaseRestSession.SignInWithEmailAsync(email, password);
            }
            catch (Exception e)
            {
                Debug.LogError("SignInWithEmail encountered an error: " + e.Message);
            ...
            OnTokenUpdated?.Invoke();
            OnLogin?.Invoke(true);
            DB().LoadUserData(FirebaseRestSession.Uid, OnFirebaseAuthenticated);
        ...
        public async void PasswordReset(string email)
        {
            try
            {
                await FirebaseRestSession.SendPasswordResetAsync(email);
            }
            catch (Exception e)
            {
                Debug.LogError("SendPasswordReset encountered an error: " + e.Message);
